...
class RenderCalendar
{
    ...
    public function __construct(\Silex\Application $app, EventFilterParams $eventFilterParams = null)
    {
    ...
    {
        if ($dateTime->format("Y") < $this->minYear) {
            throw new \Exception($this->minYear.' Onwards Only');
        }
        if ($dateTime->format("Y") > $this->maxYear) {
            throw  new \Exception('Up to '.$this->maxYear.' only');
        }
    ...
    protected function buildData()
    {
        ...
        foreach ($this->events as $event) {
            foreach ($this->eventsDataByDay as $k=>$data) {
                ...
                } elseif ($startAt < $data['startTimestamp'] && $event->getEndAt()->getTimestamp() > $data['startTimestamp']) {
