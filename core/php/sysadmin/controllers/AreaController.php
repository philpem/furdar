...
class AreaController
{
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if ($action->getCommand() == 'delete' && !$this->parameters['area']->getIsDeleted()) {
                    ...
                    $redirect = true;
                } elseif ($action->getCommand() == 'undelete' && $this->parameters['area']->getIsDeleted()) {
                    $this->parameters['area']->setIsDeleted(false);
                    $ar = new AreaRepository($app);
                    $ar->undelete($this->parameters['area'], $app['currentUser']);
                    $redirect = true;
                } elseif ($action->getCommand() == 'parentarea') {
                    $ar = new AreaRepository($app);
                    ...
                    $redirect = true;
                } elseif ($action->getCommand() == 'purge' && $app['config']->sysAdminExtraPurgeAreaPassword && $app['config']->sysAdminExtraPurgeAreaPassword == $action->getParam(0)) {
                    $ar = new AreaRepository($app);
    ...
    public function watchers($siteid, $slug, Request $request, Application $app)
    {
        $this->build($siteid, $slug, $request, $app);
