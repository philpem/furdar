        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if ($action->getCommand() == 'delete' && !$this->parameters['group']->getIsDeleted()) {
                    ...
                    $redirect = true;
                } elseif ($action->getCommand() == 'undelete' && $this->parameters['group']->getIsDeleted()) {
                    ...
                    $redirect = true;
                } elseif ($action->getCommand() == 'purge' && $app['config']->sysAdminExtraPurgeGroupPassword && $app['config']->sysAdminExtraPurgeGroupPassword == $action->getParam(0)) {
                    $gr = new GroupRepository($app);
    ...
    public function watchers($siteid, $slug, Request $request, Application $app)
    {
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                } elseif ($action->getCommand() == 'remove') {
                    $userRepo = new UserAccountRepository($app);
                    $user = $userRepo->loadByEmail($action->getParam(0));
                    if ($user) {
                        $userWatchesGroupRepo = new UserWatchesGroupRepository($app);
