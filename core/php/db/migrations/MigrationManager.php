...
class MigrationManager
{
    public static function upgrade(Application $app, bool $verbose = false)
    {
            ...
            if (is_dir($dir)) {
                ...
                while (false !== ($file = readdir($handle))) {
                    if ($file != '.' && $file != '..') {
                        ...
                        } elseif (substr($file, -strlen($fileEndingPHP)) == $fileEndingPHP) {
        ...
        foreach ($migrations as $migration) {
            if (!$migration->getApplied()) {
                if ($verbose) {
                    print "Applying ".$migration->getId()."\n";
                }
    ...
    {
        if ($a->getIdAsUnixTimeStamp() == $b->getIdAsUnixTimeStamp()) {
            return 0;
        }
