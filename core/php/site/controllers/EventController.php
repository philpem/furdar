...
class EventController
{
    ...
    protected function build($slug, Request $request, Application $app)
    {
        ...
        } elseif ($this->parameters['event']->getAreaId()) {
    ...
    public function userAttendanceHtml($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function myAttendanceJson($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editSplash($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
     ...
     * @return Response
     */
    public function editDetailsEditingJSON($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editVenue($slug, Request $request, Application $app)
    {
        //var_dump($_POST); die();
    ...
    public function editVenueSearch($slug, Request $request, Application $app)
    {
        //var_dump($_POST); die();
    ...
    public function editVenueSearchJson($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editArea($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editAreaJson($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    protected function editAreaGetDataIntoParameters(Application $app)
    {
        ...
        if ($this->parameters['search']) {
            $arb->setFreeTextSearch($this->parameters['search']);
        }
        $this->parameters['areas'] = $arb->fetchAll();
        $this->parameters['areaSearchDone'] = true;
    }

    public function editVenueNew($slug, Request $request, Application $app)
    {
        //var_dump($_POST); die();
    ...
    public function editFuture($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function recur($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function recurWeekly($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
        ...
        if ($request->request->get('submitted') == 'yes' && $app['websession']->getCSFRToken() == $app['websession']->getCSFRToken()) {
            ...
            foreach ($this->parameters['newEvents'] as $event) {
                if (in_array($event->getStartAt()->getTimeStamp(), $data)) {
                    $eventRepository->createWithMetaData(
                        $event,
                        $app['currentSite'],
                        $eventEditMetaData,
    ...
    public function recurMonthly($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function recurMonthlyLast($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function recurArbitrary($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
        ...
        if ($request->request->get('date') && $request->request->get('CSFRToken') == $app['websession']->getCSFRToken()) {
            ...
            } elseif ($eventRecurSet->isDateToLateForArbitraryDate($start, $app['timesource'])) {
                $app['flashmessages']->addError('That date is to late!');
            } else {
                $newEvents = $eventRecurSet->getNewEventOnArbitraryDateFilteredForExisting(
                    $this->parameters['event'],
                    $start->format('Y'),
                    $start->format('n'),
                    $start->format('j')
    ...
    public function cancel($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function uncancel($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function rollback($slug, $timestamp, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function moveToArea($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editTags($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editGroups($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editMedia($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
        ...
        if ($app['currentUserPermissions']->hasPermission("org.openacalendar", "CALENDAR_CHANGE")) {
            ...
            if ('POST' == $request->getMethod()) {
                ...
                if ($form->isValid() && $form['media']->getData()) {
                    ...
                    $media = $mediaRepository->createFromFile(
                        $form['media']->getData(),
                        $app['currentSite'],
                        $app['currentUser'],
                        $form['title']->getData(),
                        $form['source_text']->getData(),
                        $form['source_url']->getData()
                    );
    ...
    public function editMediaRemove($slug, $mediaslug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function editMediaAddExisting($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
    ...
    public function exportExistingGoogleCalendar($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Event does not exist.");
        }
