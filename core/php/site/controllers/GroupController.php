...
class GroupController
{
    ...
    public function setUpMainTab(Application $app)
    {
    ...
    public function importers($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Group does not exist.");
        }
        ...
        if ($app['currentUserPermissions']->hasPermission("org.openacalendar", "CALENDAR_CHANGE") && $app['currentSiteFeatures']->has('org.openacalendar', 'Group')) {
            ...
            if ('POST' == $request->getMethod()) {
                ...
                if ($form->isValid() && $form['media']->getData()) {
                    ...
                    $media = $mediaRepository->createFromFileWithMetaData(
                        $form['media']->getData(),
                        $app['currentSite'],
                        $mediaEditMetaDataModel,
                        $form['title']->getData(),
    ...
    public function editEventsTags($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Group does not exist.");
        }
    ...
    public function watch($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Group does not exist.");
        }
    ...
    public function stopWatchingFromEmail($slug, $userid, $code, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Group does not exist.");
        }
    ...
    public function newImportURL($slug, Request $request, Application $app)
    {
        if (!$this->build($slug, $request, $app)) {
            $app->abort(404, "Group does not exist.");
        }
    ...
    public function newImportURLInProgressJSON($slug, Request $request, Application $app)
    {
        if (! $this->build($slug, $request, $app)) {
            $app->abort(404, "Group does not exist.");
        }
